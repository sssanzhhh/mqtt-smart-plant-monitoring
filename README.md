...
As part of improving system robustness and real-world applicability, TLS (Transport Layer Security) encryption has been integrated into the MQTT communication layer.

All components (publishers, controllers, and dashboard) now establish secure connections to the MQTT broker using TLS over port 8883, replacing the previous unencrypted communication over port 1883.

Key Features
	•	Encrypted MQTT communication using TLS v1.2+
	•	Server certificate validation via CA certificate (ca.crt)
	•	Secure data transmission between:
	•	Publishers → Broker
	•	Broker → Controller
	•	Broker → Dashboard
	•	No changes to system architecture or data flow

Implementation Approach

TLS was integrated using the native capabilities of the paho-mqtt library without modifying the system structure:
client.tls_set(
    ca_certs="ca.crt",
    certfile=None,
    keyfile=None,
...
The encryption layer was added directly at the MQTT client initialization point in each component, ensuring minimal and non-invasive changes.

Design Principle

This enhancement follows the project’s core methodology:

Extend the system without redesigning its architecture.

TLS was introduced as a cross-cutting concern, preserving:
	•	existing OOP structure
...
WARNING
Value deviates up to 20 % beyond threshold
CRITICAL
Value deviates more than 20 % beyond threshold
...
Technologies Used
	...
	•	Pygame 2.5+
	•	JSON
	•	TLS (SSL) via Python ssl / paho-mqtt
...
MQTT-based Smart Plant Monitoring and Automatic Watering System using Python and SQLite. Implements secure 2-way publish/subscribe communication via HiveMQ broker with TLS encryption.
