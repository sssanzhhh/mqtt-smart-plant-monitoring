...
class SensorController:
    def __init__(self) -> None:
        self.sensor_key = "phosphorus"
        self.alert_type = "PHOSPHORUS_OUT_OF_RANGE"
    ...
    def _publish_alert(self, plant_id: str, plant_type: str, severity: str, value: float, low: float, high: float) -> None:
        ...
        payload = {
            ...
            "severity": severity,
            "message": f"Phosphorus out of range: {value} mg/kg (optimal {low}–{high})",
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        self.client.publish(topic, json.dumps(payload))
        print(f"[controller_phosphorus] ALERT [{severity}] {payload['message']}")
    ...
    def on_message(self, client: mqtt.Client, userdata, msg: mqtt.MQTTMessage) -> None:
        ...
        low, high = profile["phosphorus"]
        print("[controller_phosphorus]", payload.get("timestamp", time.strftime("%Y-%m-%d %H:%M:%S")), plant_id, f"phosphorus={value} mg/kg")
    ...
    def run(self) -> None:
        ...
        print(f"[controller_phosphorus] listening: {self.topic_sub}")
