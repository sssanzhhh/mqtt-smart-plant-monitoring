...
class SensorController:
    ...
    def _publish_alert(self, plant_id: str, plant_type: str, severity: str, value: float, max_allowed: float) -> None:
        ...
        payload = {
            ...
            "severity": severity,
            "message": f"Salinity too high: {value} dS/m (max {max_allowed})",
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        self.client.publish(topic, json.dumps(payload))
        print(f"[controller_salinity] ALERT [{severity}] {payload['message']}")
    ...
    def run(self) -> None:
        ...
        print(f"[controller_salinity] listening: {self.topic_sub}")
