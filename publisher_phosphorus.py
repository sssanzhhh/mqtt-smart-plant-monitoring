...
class SensorPublisher:
    def __init__(self, plant_type: str) -> None:
        ...
        low, high = PLANT_PROFILES[self.plant_type]["phosphorus"]
    ...
    def _next_value(self) -> float:
        _, high = PLANT_PROFILES[self.plant_type]["phosphorus"]
        self.value += random.uniform(-4.0, 3.5)
    ...
    def publish(self) -> None:
        ...
        print(f"[publisher_phosphorus] topic: {self.topic}")
        try:
            while True:
                payload = {
                    "plant_id": self.plant_id,
                    "plant_type": self.plant_type,
                    "phosphorus": self._next_value(),
                    "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                }
                self.client.publish(self.topic, json.dumps(payload))
                print(f"[publisher_phosphorus] sent: {payload['phosphorus']} mg/kg")
                time.sleep(1)
        except KeyboardInterrupt:
            print("\n[publisher_phosphorus] stopped")
        finally:
