...
class SensorController:
    ...
    def _publish_alert(self, plant_id: str, plant_type: str, severity: str, value: float, low: float, high: float) -> None:
        ...
        payload = {
            ...
            "severity": severity,
            "message": f"Potassium out of range: {value} mg/kg (optimal {low}–{high})",
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        self.client.publish(topic, json.dumps(payload))
        print(f"[controller_potassium] ALERT [{severity}] {payload['message']}")
    ...
    def run(self) -> None:
        ...
        print(f"[controller_potassium] listening: {self.topic_sub}")
