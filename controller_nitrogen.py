...
class SensorController:
    def __init__(self) -> None:
        self.sensor_key = "nitrogen"
        self.alert_type = "NITROGEN_OUT_OF_RANGE"
    ...
    def _publish_alert(self, plant_id: str, plant_type: str, severity: str, value: float, low: float, high: float) -> None:
        ...
        payload = {
            ...
            "severity": severity,
            "message": f"Nitrogen out of range: {value} mg/kg (optimal {low}–{high})",
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        }
        self.client.publish(topic, json.dumps(payload))
        print(f"[controller_nitrogen] ALERT [{severity}] {payload['message']}")
    ...
    def on_message(self, client: mqtt.Client, userdata, msg: mqtt.MQTTMessage) -> None:
        ...
        low, high = profile["nitrogen"]
        print("[controller_nitrogen]", payload.get("timestamp", time.strftime("%Y-%m-%d %H:%M:%S")), plant_id, f"nitrogen={value} mg/kg")
    ...
    def run(self) -> None:
        ...
        print(f"[controller_nitrogen] listening: {self.topic_sub}")
