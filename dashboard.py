...
SENSOR_LABELS = {
    ...
    "soil_ph": "Soil pH", "salinity": "Salinity", "root_temperature": "Root Temp",
}

SENSOR_UNITS = {
    "soil_moisture": "%", "temperature": "°C", "humidity": "%", "nitrogen": "mg/kg",
    "phosphorus": "mg/kg", "potassium": "mg/kg", "soil_ph": "", "salinity": "dS/m",
    "root_temperature": "°C",
}
...
def glow_rect(surface, colour, rect, radius=14, layers=4):
    ...
    for i in range(layers, 0, -1):
        ...
        pygame.draw.rect(glow, (r, g, b, alpha), (0, 0, rect[2] + expand * 2, rect[3] + expand * 2), border_radius=radius + expand)
...
def range_for_sensor(sensor_key: str, profile: dict):
    if sensor_key == "soil_moisture": return 0, 100
    if sensor_key == "temperature": return 15, 40
    if sensor_key == "humidity": return 0, 100
    if sensor_key == "nitrogen": return profile["nitrogen"]
    if sensor_key == "phosphorus": return profile["phosphorus"]
    if sensor_key == "potassium": return profile["potassium"]
    if sensor_key == "soil_ph": return profile["soil_ph"]
    if sensor_key == "salinity": return 0, profile["salinity"][1] * 1.5
    if sensor_key == "root_temperature": return profile["root_temperature"]
    return 0, 100
...
def health_colour(plant_id: str):
    ...
    if worst == 2: return C_RED
    if worst == 1: return C_YELLOW
    return C_GREEN
...
def merge_sensor_payload(payload: dict) -> None:
    ...
    if plant_id not in state:
        state[plant_id] = {"plant_id": plant_id, "plant_type": plant_type, "timestamp": payload.get("timestamp", "—")}
...
def build_reasons(plant_id, data, profile, all_alerts):
    reasons = []
    seen = set()
